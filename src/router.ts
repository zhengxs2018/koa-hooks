 ...
 * @param callback     回调函数
 */
export function route(name: string, path: string, callback: RequestMiddleware): void
 ...
 * @param callback     回调函数
 */
export function route(method: string, path: string, callback: RequestMiddleware): void
...
export function route(...args: unknown[]) {
  const [name, method, path, callback] = normalizeArgs(args)
 ...
 * @param callback     回调函数
 */
function normalizeArgs(args: any[]): [string | null, string, string, RequestMiddleware] {
  if (args.length === 4) {
